...
globalAssign = function(Y, X, Z, T_i, ni, Ga, be, s2, xi, nu) {
    V <<- Ga - xi %*% t(xi)
    V.inv <<- solve(V)
...
calc_expect_1 = function(Y, X, Z, T_i, ni, Ga, be, s2, xi, nu) {
    s = sqrt(s2)
...
be <- numeric(p)
be[1] = 1;
la = c(3); s2 = 2; Ga = matrix(c(0.35),1,1); nu = 1e5
...
repeat
{
    ...
    if (iter == 1 | (iter %% restart == 0) ) {
        ...
        for (i in 1:length(cl)) {
            clusterEvalQ(cl[i],  globalAssign(Y, X, Z, T_i, ni, Ga, be, s2, xi, nu))
        }
        estep_upd = clusterEvalQ(cl,  calc_expect_1(Y, X, Z, T_i, ni,Ga, be, s2, xi, nu))
        ...
        estep_upd = clusterEvalQ(cl[retSamp],  calc_expect_1(Y, X, Z, T_i, ni,Ga, be, s2, xi, nu))
    ...
    logli.new = -CML.covar.fn(nu,Ga, Z, FF, delta, cent, s2, tgen, dd)
...
para = list(model.inf = model.inf, beta = be, lambda = cbind(la,delta,xi), s2 = c(s=s,s2=s2), Gamma = Ga, F = FF, nu = nu,time = c(t2-t1, iter, logli.new, logli.vector), param.vector = param.vector)
#print(para)
print(t2-t1)