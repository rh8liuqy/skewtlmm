NLMM.AECM.DEC = function(Y,X,Z,N,ni,T_i, 
                      be, s2, Ga, covar_params, tol = 1e-3, max.iter = 200, per = 100)
{
  ...
  repeat
  {
    ...
    rho.optim = optim(par = covar_params, fn = CML.covar.fn, method = 'L-BFGS-B', lower = c(0.1, 0.2), upper = c(0.95,1.5),Ga = Ga, Z =Z, FF = FF, delta = delta, s2 = s2, T_i = T_i)
  ...
  delta = c(solve(FF) %*% xi)
  para = list(model.inf = model.inf, beta = be, lambda = cbind(la,delta,xi), s2 = c(s=s,s2=s2), Gamma = Ga,covar_params = covar_params, F = FF)
  cat(paste(rep('=',60),sep='',collapse=''), '\n')
